  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">情绪状态</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">疏导需求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">心理倾听与自我关怀疏导指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 情绪价值倾听与心理疏导专家');
...
const metricsList = [
  {
    "key": "empatheticListeningDepth",
    "label": "共情倾听深度"
  },
  {
    "key": "emotionalComfortWarmth",
    "label": "情绪抚慰温暖度"
  },
  {
    "key": "psychologicalReframingClarity",
    "label": "心理重塑清晰度"
  },
  {
    "key": "anxietyReliefEffectiveness",
    "label": "焦虑缓解有效性"
  },
  {
    "key": "gentleActionableAdvice",
    "label": "温和可行性建议"
  }
];
const guideRules = [
  {
    "title": "接纳所有情绪",
    "advice": "允许自己有脆弱、悲伤或愤怒的时候，情绪只是经过身体的能量，不需要强行压抑",
    "avoid": "切忌对自己进行批判或强求“必须立刻积极起来”"
  },
  {
    "title": "区分事实与想法",
    "advice": "脑海中的负面念头（如“我什么都做不好”）只是想法而非事实，学着用客体化视角观察它",
    "avoid": "避免陷入全或无、灾难化的极端思考思维"
  },
  {
    "title": "聚焦微小的掌控",
    "advice": "当感到失控时，去完成一件极小的事（如喝一杯温水、整理书桌、散步 10 分钟）",
    "avoid": "严禁在能量极低时制定宏大复杂的改变计划"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[QINGXU_SCORES\][\s\S]*?\[\/QINGXU_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[QINGXU_SCORES\](.*?)\[\/QINGXU_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('qingxu_history_records');
...
const saveHistory = () => {
  localStorage.setItem('qingxu_history_records', JSON.stringify(historyList.value));
};
